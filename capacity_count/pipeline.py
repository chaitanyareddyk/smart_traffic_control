...
class CapacityCounter(PipelineProcessor):
    ...
    def calculate_capacity(self, frame, frame_number):
        ...
        if self.save_image:
            ...
            fig = plt.figure()
            fig.suptitle("SIH-2k19    Capacity Used: {}%".format(capacity*100-17), fontsize=16)
            plt.subplot(211),plt.imshow(base_frame),plt.title('Original:')
            plt.xticks([]), plt.yticks([])
            plt.subplot(212),plt.imshow(t),plt.title('Capacity Map:')
            plt.xticks([]), plt.yticks([])

            fig.savefig(self.image_dir + ("/processed_%04d.png" % frame_number), dpi=500)
    ...
    def __call__(self, context):
        ...
        self.log.debug("Capacity Used: {}%".format(capacity*100-17))
